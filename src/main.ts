...
export default class QuipPlugin extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			...
			checkCallback: (checking: boolean) => {
				...
				if (markdownView) {
					// If checking is true, we're simply "checking" if the command can be run.
					// If checking is false, then we want to actually perform the operation.
					const link = this.app.metadataCache.getFileCache(
		...
		this.addCommand({
			...
			callback: () => {
				try {
					const client = new QuipAPIClient(
	...
	async publishHTML(markdownView: MarkdownView, title: string) {
		...
		try {
			...
			const response = await client.newHTMLDocument(
				html,
				title,
	...
	async updateHTML(link: string, markdownView: MarkdownView) {
		...
		try {
			...
			new SuccessModal(
				this.app,
				link,
				`If your Quip document hasn't updated yet, try refreshing.`
			).open();
		} catch (error) {
	...
	async tryPreload(): Promise<QuipThread[]> {
		...
		try {
			...
			for (const [thread_id, thread_response] of Object.entries(
				await client.getRecentThreads()
			)) {
