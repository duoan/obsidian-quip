...
export class QuipAPIClient {
	...
	async newHTMLDocument(
		html: string,
		title: string,
		member_ids: string
	): Promise<QuipThreadResponse> {
	...
	async updateHTMLDocument(
		...
		const current_html = (await this.prependHTML(secret_path, prepend_html))
			.html;
		...
		for (const section_header of Array.from(section_headers)) {
			if (section_header.getText() == marker) {
				marker_found = true;
				console.debug(
					`Found deletion marker: ${section_header.getText()}`
				);
			} else if (marker_found) {
				console.debug(
					`Deleting h1 header: ${section_header.getText()}`
				);
				promises.push(
					this.deleteSection(
						secret_path,
						section_header.getAttr("id")
					)
				);
			}
		}
	...
	async getThreadHTML(
		options: GetThreadHTMLArguments
	): Promise<QuipThreadHTMLResponse> {
		let url = `/2/threads/${options.threadIdOrSecretPath}/html`;
		if (options.cursor) {
			url += `?cursor=${options.cursor}`;
		}
		return this.api<Record<string, string>, QuipThreadHTMLResponse>(
			url,
			null
		);
	}
	...
	async appendHTML(
		secret_path: string,
		html: string
	): Promise<QuipThreadEditResponse> {
	...
	async editDocument(
		options: EditDocumentArguments
	): Promise<QuipThreadEditResponse> {
		return this.api<EditDocumentArguments, QuipThreadEditResponse>(
	...
	async getBlob(path: string): Promise<Blob> {
		...
		if (status >= 400) {
			switch (status) {
				...
				default:
					throw new Error(
						`Quip server error: ${status}: ${response.text}`
					);
			}
		}
	...
	async getThread(
		thread_id_or_secret_path: string
	): Promise<QuipThreadResponse> {
	...
	async newDocument(
		options: NewDocumentArguments
	): Promise<QuipThreadResponse> {
		return this.api<NewDocumentArguments, QuipThreadResponse>(
			"/1/threads/new-document",
			options
		);
	}

	async searchTitles(query: string): Promise<QuipThreadResponse[]> {
		const url = `/1/threads/search?only_match_titles=true&query=${encodeURIComponent(
			query
		)}`;
		return this.api<Record<string, string>, QuipThreadResponse[]>(
			url,
			null
		);
	}

	buildRequest<ArgType extends Record<string, string>>(
		path: string,
		postArguments: ArgType
	): RequestUrlParam {
	...
	async api<ArgType extends Record<string, string>, ResponseType>(
		path: string,
		postArguments: ArgType
	): Promise<ResponseType> {
		const resource = this.buildRequest(path, postArguments);
		const response = requestUrl(resource);
		console.debug(`Quip API request: ${resource.url}`);
		...
		if (status >= 400) {
			switch (status) {
				...
				default:
					throw new Error(
						`Quip server error: ${status}: ${await response.text}`
					);
			}
		}
